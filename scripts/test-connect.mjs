...
const outDir = resolve(root, ".tmp-test-connect");
...
await build({
  ...
  build: {
    ...
    emptyOutDir: true,
    lib: {
      entry: resolve(root, "src/shared/connect-json-parser.ts"),
      formats: ["es"],
      fileName: () => "connect-json-parser",
    },
    rollupOptions: {
      output: {
        inlineDynamicImports: true,
        entryFileNames: "connect-json-parser.js",
      },
    },
  },
  logLevel: "error",
...
const { ConnectBinaryFramer, ConnectJsonParser } = await import(
  `file:///${resolve(outDir, "connect-json-parser.js").replace(/\\/g, "/")}`
);
...
console.log("connect-json-parser tests passed");
