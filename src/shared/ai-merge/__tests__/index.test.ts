...
describe("ai-merge", () => {
  it("matches previous script coverage", () => {
    ...
    // Real captures under data/ (optional locally; skip if missing)
    {
      ...
      function loadKimiJsonStream(filePath: string) {
      ...
      function loadSse(filePath: string) {
        const text = readFileSync(filePath, "utf8");
      ...
      if (existsSync(deepseekPath)) {
        ...
        assert(
          t.channels.content.includes("啊啊"),
          `content snip: ${t.channels.content.slice(0, 40)}`,
        );
        assert(
      ...
      if (existsSync(deepseekSearchPath)) {
        ...
        assert(
          t.channels.content.includes("今天首尔很热"),
          `search content: ${t.channels.content}`,
        );
      }
      ...
      if (existsSync(doubaoPath)) {
        ...
        assert(
          t.channels.tools.length === 1,
          `doubao search tool count ${t.channels.tools.length}`,
        );
      ...
      if (existsSync(yuanbaoPath)) {
        ...
        assert(
          yArgs.results.some(
            (r) =>
              String(r.site_name || "").includes("气象") || String(r.url || "").includes("http"),
          ),
